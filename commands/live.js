...
const { yantablo, yantablo2, live, live2, mvp, mvp2, slot, slot2, win } = require('../config/tabloisim.json')
...
module.exports = {
  name: 'live',
  description: 'Live tablosu oluşturur.',
  async execute(message, args) {
      ...
      if (embed.fields.length === 0) {
          ...
          collector.on('collect', async (reaction, user) => {
              const channel = message.guild.channels.cache.get(livekanalId[0]);
              if (channel) {    
